...
def process_reads(bamfile_path, sample, contig, start, end,read_index_start , read_index_end):
    ...
    for idx, read in enumerate(bamfile.fetch(contig, start, end, until_eof=True)):
        if idx >= read_index_start:
            if idx <= read_index_end:
...
def main():
    ...
    # Split BAM based on depth, now we get chunks containing reads
    bamfile = pysam.AlignmentFile(bamfile_path, "rb")


    # Count the number of reads in the specified region
    read_count = bamfile.count(contig, start, end)
    # Chunk size for processing reads
    chunk_size = 5000  # Process 5,000 reads at a time (adjust as needed)
    # list of chunk starts and ends
    chunk_starts = []
    chunk_ends = []
    for i in range(0, read_count, chunk_size):
    # Calculate the reads in chunk
        chunk_starts.append(0 + i)
        chunk_ends.append(min(i + chunk_size, read_count - 1))
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        future_to_chunk = {executor.submit(process_reads, bamfile_path, sample, contig, start, end,read_index_start , read_index_end): (read_index_start, read_index_end) 
                           for read_index_start, read_index_end in zip(chunk_starts, chunk_ends)}
    ...
    result_df = restructure_paired_end(final_df)
