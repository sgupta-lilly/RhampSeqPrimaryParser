...
def classify_read(read_edit_quality):
    classify_IDX = {'I': [], 'D': []}
    ...
    for key in read_edit_quality:
        if key == 'I':
            ...
            if read_edit_quality[key] != []:
                if (read_edit_quality[key][0]) > 29:
                   classify_IDX[key].append("I_High")
                elif (read_edit_quality[key][0]) <= 29:   
...
def extract_indels_and_mismatches(read):
    ...
    for cigar_op, length in read.cigar:
        if cigar_op == 4:  # Soft clip (S)
            # SoftClip move the reference but not the read position
            read_pos = length
            
        elif cigar_op == 0:  # Match (M) or mismatch
            ref_pos += length
            read_pos += length
            
        elif cigar_op == 1:  # Insertion (I)
            # Capture insertion: Note that insertions happen in the read, not the reference
            quality = base_qualities[read_pos :read_pos  + length]            
            error_P = [10 ** (-q / 10) for q in quality]  # Probability of error for insertion bases
            indels_mismatches['I'].append((ref_pos, length, quality, error_P))
            read_pos += length
            read_edit_quality['I'].append(min(quality))
        elif cigar_op == 2:  # Deletion (D)
            # Capture deletion: Deletions move the reference but not the read position
            quality_1 = base_qualities[read_pos - 1] if read_pos > 0 else None
            quality_2 = base_qualities[read_pos + 1] if read_pos > 0 else None
            quality = quality_1 if quality_1 < quality_2 else quality_2
            ...
            read_edit_quality['D'].append(quality)
                
    ###########MISMATCHES############
    # Parse MD tag for mismatches (X)
    #location of mimatch in the read to get the base quality
    #accounting for soft clipped bases. These bases are present in the read but not in the reference
     # Reset read position for MD tag parsing
    for cigar_op, length in read.cigar:
        if cigar_op == 4:
            read_pos = length
        else:
    ...
    #else:
    #    read_pos = 0 
    #MD tag has SNP information
    md_tag = read.get_tag('MD')
    mismatches = re.findall(r'(\d+|[A-Z]|\^[A-Z]+)', md_tag)
    mismatch_quality = []
    mismatch_positions = []
    mismatch_error_P = []
    current_position = read_pos
    #location of the mismatch in the reference sequence
    reference_mismatch_positions = [index for index, char in enumerate(read.get_reference_sequence(), start=read.reference_start) if char.islower()]
    for item in mismatches:
        if item.isdigit():
        # Add the number of matching bases to the current position            
            current_position += int(item)
        elif item.startswith("^"):
        # Ignore deletions; do not process or report them here
            continue
        else:
        # For mismatch, record the current position        
            current_position += 1  # Advance the position by 1 for the mismatch itself
            if 0 <= current_position < len(read.query_qualities):
                quality = read.query_qualities[current_position]
                error_P = 10 ** (-quality / 10)
            else:
                quality = None
                error_P = None
            mismatch_positions.append(current_position)
            mismatch_quality.append(quality)
            mismatch_error_P.append(error_P)
    #print(str(md_tag)+"\t"+str(read.cigarstring)+"\t"+str(read.reference_start)+"\t"+str(mismatch_positions)+"\t"+str(mismatch_quality)+"\t"+str(reference_mismatch_positions))
    for i in range(0, len(reference_mismatch_positions)):
        indels_mismatches['X'].append((reference_mismatch_positions[i], 1, mismatch_quality[i], mismatch_error_P[i]))
        read_edit_quality['X'].append(quality)
...
def merge_to_bam(read_sequence, mate_sequence, min_overlap=2, quality_score=30):
    ...
    for i in range(max_len - min_overlap + 1):
        ...
        if identity >= 0.5:
            ...
            overlap_start_seq1 = i  # Position where overlap starts in seq1
            overlap_end_seq1 = i + overlap_len  # Position where overlap ends in seq1
            overlap_start_seq2 = 0  # Position where overlap starts in seq2 (always 0 in the overlap portion)
            overlap_end_seq2 = overlap_len  # Position where overlap ends in seq2
            
            #print(overlap_start_seq1, overlap_end_seq1,overlap_start_seq2, overlap_end_seq2,overlap_len, 1 )
            return overlap_start_seq1, overlap_end_seq1,overlap_start_seq2, overlap_end_seq2,overlap_len, 1  # Return 1 indicating that the merge was successful
...
def process_reads(bamfile_path, sample, contig, start, end,chunk):
    ...
    columns = [
        ...
        "QC_Fail", "Secondary", "Supplementary", "Duplicate", "Insertions", "Deletions", "Mismatches", "Sequence", "Read_1", "Read_2", "INS_QC", "DEL_QC", "ReadQualities", "ReferencePosition"
...
def process_merge_for_reads(df):
    ...
    for idx, merged_row in df.iterrows():
        ...
        if read_sequence != 'NA' and mate_sequence != 'NA':
            overlap_start_seq1, overlap_end_seq1,overlap_start_seq2, overlap_end_seq2, overlap_len, merge_status = merge_to_bam(read_sequence, mate_sequence, min_overlap=2, quality_score=30)
            
        else:
            overlap_start_seq1 = 0
            overlap_end_seq1 = 0
            overlap_start_seq2 = 0
            overlap_end_seq2 = 0
            overlap_len = 0
            merge_status = 0  # No merge if one or both sequences are 'NA'

        # Append the results to the lists
        seq1_overlap.append(str(overlap_start_seq1)+"-"+str(overlap_end_seq1))
        seq2_overlap.append(str(overlap_start_seq2)+"-"+str(overlap_end_seq2))
        overlap_lens.append(overlap_len)
        merge_successes.append(merge_status)

    # Add the results as new columns in the dataframe
    df['Seq1_Overlap'] = seq1_overlap
    df['Seq2_Overlap'] = seq2_overlap
